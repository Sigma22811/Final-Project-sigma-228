...
def main():
    ...
    while keepplaying:
        ...
        if money <= 0:
            print("You're out of money! no gambling anymore...")
            game_over()
            keepplaying = False
        else:
            print("Your money is now " + str(money) + " Gold.")
            ask = input("Do you want to keep gambling? [y/N] >> ")
            if ask == 'y' or ask == 'Y':
                keepplaying = True
            else:
                print("You're leaving with " + str(money) + " Gold. Adios!.")
                game_over()
                keepplaying = False
